    ...
    public ArrayList<Pair<Integer, Location>> getTrajectory(int currentTime, Pair<Integer, Location> srcEntry, Location dst) {
        ...
        if (this.connectType == ConnectType.VERTICAL) {
            if (n1.contains(src)) {
                ...
                traj.add(new Pair<Integer,Location>(stepInTime - currentTime, connectPoint[1]));
                return traj;
            } 
            // n2 contains src
            ...
            traj.add(new Pair<Integer,Location>(stepInTime - currentTime, connectPoint[0]));
            return traj;
        }
        ...
        if (nTo.getNodeType() == NodeType.RACK && nTo.contains(dst)) {
            ...
            traj.add(new Pair<Integer, Location>(timeOffset, src));
        }
        ...
        traj.add(new Pair<Integer,Location>(stepInTime - currentTime, dst2));
        return traj;
    }
