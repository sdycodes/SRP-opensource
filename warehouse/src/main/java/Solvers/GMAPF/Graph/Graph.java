...
public class Graph {
    ...
    public Path planning(int currentTime, Location src, Location dst) {
        ...
        dist[srcNodeId] = 0;
        entryPoint[srcNodeId] = new Pair<Integer, Location>(0, src);
        preNodeId[srcNodeId] = -1;
        pq.add(new Pair<Integer, Integer>(srcNodeId, 0));
        while (!pq.isEmpty()) {
            ...
            for (Edge e: adj[curNodeId]) {
                ...
                if (dist[nextNodeId] == -1 || dist[nextNodeId] > dist[curNodeId] + weight) {
                    ...
                    pq.add(new Pair<Integer, Integer>(nextNodeId, dist[nextNodeId]));
                }
            }
        }
        // parse to path
        ...
        if (nodeList.get(0) != srcNodeId) {
            ...
            traj.add(new Pair<>(0, new SimulatorCore.Location(src.getRow(), src.getCol())));
            traj.add(new Pair<>(src.manhattanDistance(dst), new SimulatorCore.Location(dst.getRow(), dst.getCol())));
        ...
        for (int i = 1; i < nodeList.size(); ++i) {
            ...
            for (int j = 0; j < preTraj[endNodeId].size(); ++j) {
                ...
                trajectory.add(new Pair<Integer, Location>(t, new Location(loc.getRow(), loc.getCol())));
            }
        }
        return new Path(trajectory.stream().map(o -> new Pair<Integer, SimulatorCore.Location>(o.getKey(), new SimulatorCore.Location(o.getValue().getRow(), o.getValue().getCol()))).collect(Collectors.toCollection(ArrayList::new)), currentTime);
    }
